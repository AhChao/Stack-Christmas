...
const translations = {
  zh: {
    ...
    aiDisclaimer: '註：本款遊戲的美術皆由 Gemini AI 生成',
    soloDisabled: '單人模式擁有專屬規則'
  },
  en: {
    designer: 'Designer',
    players: 'Players',
    ...
    aiDisclaimer: 'Note: All game art is generated by Gemini AI',
    soloDisabled: 'Solo mode has its own rule to play'
  }
};
...
import pveImg from '../assets/pve.png';
import soloImg from '../assets/solo.png';

const diffData = {
  beginner: { img: noviceImg },
  normal: { img: normalImg },
  hard: { img: hardImg },
  expert: { img: expertImg }
};

const diffOrder = ['beginner', 'normal', 'hard', 'expert'];
...
const handleStartClick = () => {
  ...
  } else if (selectedMode.value === 'solo') {
    emit('start', { 
      mode: 'solo',
      randomBoard: false, // Solo mode uses fixed boards
      paidMulligan: true, // Always enabled in solo mode
      lang: lang.value
    });
  } else {
      ...
      <div class="content-scroll">
        ...
        <template v-else-if="state === 'select-mode'">
          <div class="selection-view">
            ...
            <!-- New Options -->
            <div class="extra-options" :class="{ 'solo-disabled': selectedMode === 'solo' }" :data-tooltip="t.soloDisabled">
              <label class="custom-checkbox" :class="{ 'disabled-pointer': selectedMode === 'solo' }">
                <input type="checkbox" v-model="randomInitialBoard" :disabled="selectedMode === 'solo'">
              ...
              <label class="custom-checkbox mt-15" :class="{ 'disabled-pointer': selectedMode === 'solo' }">
                <input type="checkbox" v-model="paidMulliganEnabled" :disabled="selectedMode === 'solo'">
                <span class="checkmark"></span>
                <div class="checkbox-text">
        ...
        <template v-else-if="state === 'select-difficulty'">
          <div class="selection-view">
            ...
            <div class="difficulty-options" :class="{ 'grid-2': state === 'select-difficulty' }">
              <div 
                v-for="key in diffOrder" 
                :key="key"
                class="choice-card"
              ...
              >
                <div class="image-wrapper">
                  <img :src="diffData[key].img" :alt="key" />
                </div>
                <div class="nameplate">
                  {{ t.diffLabels[key] }}
                </div>
...
.extra-options.solo-disabled::after {
...
.extra-options.solo-disabled:hover::after {
  opacity: 1;
}
